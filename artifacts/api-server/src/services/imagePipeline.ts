...
4. 如果有 styleProfile，把客户偏好（色调/布局/emoji/情绪）融进 prompt
5. prompt 中英混合，详细具体（构图、色调、光线、材质、镜头感）
6. **不要让 AI 画文字** —— 文字单独叠加，prompt 里写 "no text, no words, no logo, no letters"
7. emojisToInclude 输出建议在画面上点缀的 emoji（来自参考图 emojis + 适合新主题的）
8. 推荐尺寸: 小红书封面优选 1024x1536 (3:4竖版)
