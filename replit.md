...
- **品牌画像统一注入 (`services/brandContext.ts` → `loadBrandContext(userId, platform)`):** 单一公共 helper,从 `brand_profiles` 加载 (category/products/audience/priceRange/tone/forbiddenClaims/conversionGoal),返回 `{ promptBlock, forbiddenClaims, brand }`(promptBlock 1500 字截断,失败/空配置返回空字符串永不阻断)。**已注入端点:** `/ai/rewrite`、`/ai/generate-title`、`/ai/generate-hashtags`、`/ai/generate-image`(同时用 `brandStyleHint` 替换硬编码"小红书爆款封面")、`/ai/generate-image-pipeline`(传 brandBlock 给 `generateImagePrompt`)、`/ai/refine-schedule-item`,以及 `/ai/generate-video`、`/ai/generate-video-sora`(传 brandBlock 给 `enqueueVideoJob` → 序列化进 `video_jobs.input` → worker `generateVideoCreativePlan` 透传)。`autopilot strategy approve` 同时把 brandBlock 注入 video enqueue + image kickOffImageForDraft(新增 `userId` 参数)。`/ai/generate-image` 同时按 `platform` 参数动态化 `platformLabel`(小红书/TikTok/Instagram/Facebook),不再硬假设 XHS。
- **Sora Pro 定价 + 日上限:** `CREDIT_COSTS["ai-generate-video-sora"]` = **500** 积分(原 250 严重亏本:1080p 12s ≈ $6 ≈ 43 RMB 上游成本,1 RMB ≈ 10 积分)。第二道闸门:`/ai/generate-video-sora` enqueue 前按用户/24h 计数 `video_jobs.input->>'provider'='sora-pro'` 命中 `SORA_DAILY_LIMIT_PER_USER`(默认 3) → `429 { error: "sora_daily_limit", used, limit }`。admin 豁免。
- **Dashboard `/api/dashboard/recent-activity` 500 修复:** 之前 row.action(不存在,DB 字段是 `type`) → undefined → zod parse 必崩 500。改为 `row.type` + 兜底 unknown,同时 openapi `ActivityItem.type` 去掉过窄 enum 改纯 string(支持新加的 `strategy.generated/approved` 等)。**改 openapi.yaml 后必跑 `pnpm --filter @workspace/api-spec run codegen`** 重生成 zod。
- **多实例风险 warning (index.ts):** `REPLIT_DEPLOYMENT=1 && !REDIS_URL` → 启动 `logger.warn` 提示 aiRateLimit/cron/in-flight 是单实例语义,部署 autoscale 必须 maxInstances=1 或外置 Redis。这是 warning 而非硬阻断,因为 dev 不需要。
