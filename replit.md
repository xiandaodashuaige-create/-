...
- **Admin Panel:** Provides user and credit management, transaction history, and system statistics for administrators. **Initial admin emails sourced from `INITIAL_ADMIN_EMAILS` env var (comma-separated)** — only applied at first signup; existing user roles read from `users.role` DB column.
- **OAuth Token at-rest encryption:** All `oauth_access_token` / `oauth_refresh_token` values in `accounts` table are AES-256-GCM encrypted via `lib/crypto.ts` (prefix `enc:v1:<iv>:<tag>:<cipher>`). Requires `OAUTH_TOKEN_ENCRYPTION_KEY` (32-byte hex). Backward compatible: plaintext rows decrypt as-is; on next OAuth callback / token refresh they are re-written encrypted. If key missing, falls back to plaintext with warn log.
- **Publish Logs (`publish_logs` table):** Each publish attempt (success/failed) writes a row with `scheduleId`, `attempt`, `status`, `postId`, `errorMessage`, `durationMs` for historical analytics — independent of the `schedules` row's mutable status. Inserted from `publishDispatcher` after `markPublished` / `markFailed`.
- **Whisper Competitor Transcription:** `POST /api/competitor-posts/:id/transcribe` downloads the post's `mediaUrl` and runs OpenAI `whisper-1` to extract spoken text → cached in `competitor_posts.transcript`. Costs 1 ai-operations-rewrite credit. Used to extract viral hooks for the AI strategy generator.
- **TikTok Provider Switcher (`services/tiktokProviders/`):** `TIKTOK_DATA_PROVIDER` env (default `tikhub`) selects active provider. Stubs for `echotik` / `fastmoss` / `kalodata` exist as adapter scaffolding — each requires its own paid API key and adapter implementation before use.
- **Schedule Timezone Fix:** `combineDateTime(startDate, dayOffset, time, tz)` parses scheduled-at in the user's timezone (default `Asia/Shanghai`) instead of server local TZ. Frontend sends `Intl.DateTimeFormat().resolvedOptions().timeZone` automatically with bulk-create.
- **Ayrshare profileKey 哨兵值修复 (P0):** `/oauth/ayrshare/sync` 把账号写入时 `ayrshareProfileKey="default"` 作为"走 Ayrshare 默认 profile"的标记。`publishDispatcher` 透传 profileKey 时增加 guard：`row.ayrshare_profile_key !== "default"` 才作为真实 `Profile-Key` header 透传，否则传 undefined（让 Ayrshare 走默认 profile）。避免 Ayrshare 收到 `Profile-Key: default` 报 404。
- **content.ownerUserId 直挂 + LEFT JOIN：** `content` 表新增 `owner_user_id` 字段（`onDelete: cascade` to users）+ 索引 `content_owner_idx`。`/api/content` 列表查询从 INNER JOIN accounts 改为 LEFT JOIN + `WHERE c.owner_user_id = ?`，`mapContentRow` 返回 `accountDeleted` 布尔字段。删账号后历史内容仍可见，前端展示"账号已删除"灰色徽标。已对存量数据做 `UPDATE content SET owner_user_id = (SELECT owner_user_id FROM accounts WHERE id = content.account_id)` 回填。
- **Whisper 口播提取前端集成：** `/competitors` 视频帖卡片底部加"提取口播"按钮（仅 mp4/mov/m3u8 可见），调 `POST /api/competitor-posts/:id/transcribe`，结果缓存到 `competitor_posts.transcript`，再次点击直接打开"查看口播"弹窗（含一键复制）。耗 1 ai-operations-rewrite 积分，按需手动触发，不在策略生成时自动批量调。
- **Hardened API server runtime (P0 batch A):** `src/config/env.ts` validates required env vars (PORT/DATABASE_URL/CLERK_*/SESSION_SECRET) at boot via zod and `process.exit(1)` if missing — warns (no-exit) on optional integrations (OPENAI/ARK/META/TIKTOK keys). `src/middlewares/errorHandler.ts` is a global tail middleware that catches `ApiError` (custom), `ZodError` (validation), and unhandled errors → uniform `{success:false, error:{code,message,details?}}` response with stack hidden in production. `src/types/express.d.ts` augments `Express.Request` with `userId / dbUser / creditOperation / creditCost` so the codebase no longer relies on `(req as any)`. `/api/readyz` joins `/api/healthz` with a 3s DB ping for K8s readiness / post-deploy smoke. `schedules.updatedAt` added with `$onUpdate(() => new Date())` for status-change auditability.
